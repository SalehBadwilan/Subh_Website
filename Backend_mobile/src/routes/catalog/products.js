...
export default function createProductRoutes({ models, env }) {
  const router = Router();
  ...
  const buildImageUrl = (url) => {
    ...
    // إذا كان الرابط مخزن كامل (قديم)
    if (url.startsWith("http://") || url.startsWith("https://")) {
      return url;
    }

    // إذا كان الرابط نسبي (جديد)
    return `${baseUrl}${url}`;
  };
  ...
  return {
    ...
    image_url: primary ? buildImageUrl(primary.url) : null,
