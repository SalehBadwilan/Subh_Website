...
export default function createCustomerProductRoutes({ models }) {
  ...
  const buildImageUrl = (url) => {
    ...
    // إذا الرابط كامل (البيانات القديمة)
    if (url.startsWith('http://') || url.startsWith('https://')) {
      return url.replace(/^https?:\/\/127\.0\.0\.1:3000/, baseUrl);
    }

    // إذا الرابط نسبي (البيانات الجديدة)
    return `${baseUrl}${url}`;
  };
  ...
  return {
    ...
    image_url: primaryImage ? buildImageUrl(primaryImage.url) : null,

    images: (p.ProductImages || []).map((i) => ({
      id: i.id,
      image_url: buildImageUrl(i.url),
