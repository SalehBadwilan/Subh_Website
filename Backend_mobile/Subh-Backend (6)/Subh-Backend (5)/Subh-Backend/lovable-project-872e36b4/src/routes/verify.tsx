...
import { api, ApiError, type ApiOk } from "@/lib/api-client";
...
function VerifyPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (!phone) {
      setError("انتهت الجلسة. عُد إلى شاشة الدخول لإعادة إرسال الرمز.");
      return;
    }
    setError(null);
    setLoading(true);
    try {
      // POST /api/auth/otp/verify → { ok, data: { user, token, token_expires_in } }
      ...
      >("/api/auth/otp/verify", { phone, otp: code }, undefined);

      const token = res?.data?.token;
      if (!token) {
        throw new ApiError(0, "استجابة غير متوقعة من الخادم.");
      }
      // Persist both the legacy flag and the real JWT.
      ...
      if (err instanceof ApiError) {
        // Backend returns 422/400 for bad code format, 400/403/404 for wrong
        // code / expired / unknown phone. Surface its message verbatim.
  ...
  async function handleResend() {
    ...
    setError(null);
    try {
      await api.post("/api/auth/otp/request", { phone }, undefined);
    } catch (err) {
      if (err instanceof ApiError) {
        setError(err.message);
      } else {
        setError("تعذّر إعادة إرسال الرمز.");
      }
    }
    setResending(false);
    setCode("");
