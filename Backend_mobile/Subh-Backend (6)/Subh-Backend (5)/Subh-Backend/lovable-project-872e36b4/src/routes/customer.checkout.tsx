...
function CheckoutPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    lines.map((l) => ({
      ...
      merchantId: l.product.merchantId,
    }))
  );
    ...
    // Guard: the backend only accepts REAL product UUIDs. The prototype's mock
    // product ids (p1, b1, ...) are not valid backend ids, so we stop here with
    // a clear message rather than letting the request fail with a confusing 404.
    const invalidProducts = lines.filter((l) => !isValidBackendProductId(l.product.id));
    if (invalidProducts.length > 0) {
      toast.error("الدفع الحقيقي يتطلب منتجات من المتجر", {
        description:
          "السلة الحالية تحتوي على منتجات تجريبية. أضف منتجات حقيقية من المتجر لإتمام الدفع.",
      });
      return;
    }

    if (!isValidBackendProductId(addressId)) {
      toast.error("اختر عنوانًا محفوظًا حقيقيًا", {
        description: "عنوان التوصيل الحالي تجريبي. أضف عنوانًا من صفحة العناوين لإتمام الدفع.",
      });
      return;
    }
    ...
    setPlacing(true);
    try {
      // Real backend flow: create order → initiate payment → confirm payment.
      const cardLast4 = card.number.replace(/\s+/g, "").slice(-4) || "4242";

console.log("LINES:", lines);

const orderLines = lines.map((l) => ({
  product_id: l.product.id,
  merchant_id: l.product.merchantId,
  quantity: l.qty,
}));

console.log("ORDER LINES:", orderLines);

const result = await checkoutWithBackend({
  ...
  card: { last4: cardLast4 },
      ...
      // Mirror the result into the local store so the Orders page (still mock
      // for now) can show the just-placed order immediately. The source of
      ...
      addNotification({
        ...
        body: `تم استلام الدفع لطلب ${result.orderNumber}.`,
      });

      toast.success("تم الدفع بنجاح", {
        description: `طلبك ${result.orderNumber} أُنشئ.`,
      });
      ...
      const message =
        err instanceof ApiError ? err.message : "تعذّر إتمام عملية الدفع. حاول مرة أخرى.";
      toast.error("فشل إتمام الدفع", { description: message });
    } finally {
      setPlacing(false);
    }
  }
