...
export const merchantEmployeeNav: NavItem[] = [
  { to: "/merchant-employee", label: "لوحة التحكم", icon: LayoutDashboard, exact: true },
  { to: "/merchant-employee/orders", label: "الطلبات المسندة", icon: ClipboardList },
  { to: "/merchant-employee/products", label: "المنتجات المسندة", icon: Package },
  { to: "/merchant-employee/inventory", label: "المخزون", icon: Warehouse },
  { to: "/merchant-employee/reports", label: "التقارير", icon: BarChart3 },
];

export function MerchantEmployeeShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-dvh bg-muted/30">
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-border bg-background/95 px-4 py-3 backdrop-blur lg:hidden">
        <Link to="/merchant-employee" className="flex items-center gap-2">
          <BrandMark className="h-8 w-8 text-primary" />
          <span className="text-lg font-extrabold text-foreground">صبح • موظف</span>
        </Link>
        <button
          type="button"
...
export function MerchantEmployeePage({
  title,
  subtitle,
  action,
  children,
