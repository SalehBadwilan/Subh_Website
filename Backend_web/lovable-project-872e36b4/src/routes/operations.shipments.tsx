...
function OperationsShipmentsPage() {
  ...
  function next(o: Order) {
    const s = opsOf(o);
    if (s === "preparing") {
      updateOrderOpsStatus(o.id, "ready");
      toast.success("الطلب جاهز للشحن.");
    } else if (s === "ready") {
      updateOrderOpsStatus(o.id, "out_for_delivery");
      toast.success("خرج الطلب للتوصيل.");
    } else if (s === "out_for_delivery") {
      updateOrderOpsStatus(o.id, "delivered");
      toast.success("تم توصيل الطلب.");
    }
  }

  function labelForNext(o: Order): string | null {
    const s = opsOf(o);
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground num">
                    {o.itemCount} منتجات • {formatSAR(o.total)}
                  </p>
                </div>
