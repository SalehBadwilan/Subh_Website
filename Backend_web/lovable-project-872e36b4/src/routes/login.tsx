...
function LoginPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setLoading(true);
    setError(null);
    // Normalize the phone to the international format the backend expects.
    const normalizedPhone = `+966${digits}`;
    try {
      sessionStorage.setItem("subh:phone", normalizedPhone);
    } catch {
      /* ignore */
    }

    // Hit the real OTP request endpoint. The backend stores the OTP and (in
    // dev) returns it as devOtp so the verify page can auto-display it. If the
    // backend is unreachable, we still navigate to /verify so the user can
    // retry — the verify page will surface a clear error there.
    try {
      await api.post("/api/auth/otp/request", { phone: normalizedPhone }, undefined);
    } catch (err) {
      // Network/500 etc. — keep going to verify so the error is shown there
      // with a retry option, rather than blocking the whole login flow.
      if (err instanceof ApiError) {
        // 422 = bad phone format from the backend's validator.
