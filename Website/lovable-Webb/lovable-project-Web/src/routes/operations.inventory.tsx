...
const [loading, setLoading] = useState(true);
  ...
  const filtered = useMemo(() => {
    return catalog.filter((p) => {
      if (onlyLow && p.on_hand > LOW) return false;
      if (
        q &&
        !p.catalog?.name_ar?.toLowerCase().includes(q.toLowerCase()) &&
  ...
  reason.trim() || (dir > 0 ? "زيادة" : "خصم"),
    ...
    toast.success(
      dir > 0
        ...
        : `تم خصم ${n} من مخزون «${active.catalog?.name_ar ?? active.sku}».`,
    );
    setActive(null);
    setDelta("");
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {movementReasonMap[m.reason] ?? m.reason} •{" "}
              ...
              <div className="space-y-4">
                <div className="rounded-xl border border-border bg-muted/40 p-3">
                  <p className="text-sm font-bold text-foreground">
                    {active.catalog?.name_ar ?? active.sku}
                  </p>
