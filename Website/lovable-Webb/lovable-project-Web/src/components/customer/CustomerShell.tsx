    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/70">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3 sm:gap-4 sm:py-4">
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          <IconLink to="/customer/ai-search" label="البحث الذكي">
            <Sparkles className="h-5 w-5" />
          </IconLink>
          <IconLink to="/customer/notifications" label="الإشعارات" badge={unreadNotifications}>
            <Bell className="h-5 w-5" />
          </IconLink>
          <IconLink to="/customer/cart" label="السلّة">
            <span className="relative">
              <ShoppingCart className="h-5 w-5" />
              {cartCount > 0 && (
                <span className="num absolute -left-2.5 -top-2 grid h-4 min-w-4 place-items-center rounded-full bg-destructive px-1 text-[10px] font-bold text-destructive-foreground ring-2 ring-background">
                  {cartCount}
                </span>
              )}
            </span>
...
export function MobileBottomNav() {
  ...
  const items: {
    ...
    },
    {
      ...
      match: (p) => p.startsWith("/customer/categor"),
    },
    {
      label: "السلّة",
      icon: ShoppingCart,
      to: "/customer/cart",
      match: (p) => p.startsWith("/customer/cart") || p.startsWith("/customer/checkout"),
      badge: cartCount,
    },
    {
      label: "طلباتي",
    ...
    },
    {
      ...
      match: (p) =>
        ...
        p.startsWith("/customer/addresses") ||
        p.startsWith("/customer/support"),
    },
  ];
    ...
    <footer className="mt-4 rounded-3xl border border-border bg-card px-6 py-8 text-sm text-muted-foreground">
      <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap gap-4 text-xs">
          <Link to="/customer/support" className="hover:text-foreground">
            الدعم
          </Link>
          <Link to="/customer/orders" className="hover:text-foreground">
            طلباتي
          </Link>
          <Link to="/customer/addresses" className="hover:text-foreground">
            العناوين
          </Link>
        </div>
        <p className="text-xs">© {new Date().getFullYear()} صبح. جميع الحقوق محفوظة.</p>
      </div>
    </footer>
  );
}
